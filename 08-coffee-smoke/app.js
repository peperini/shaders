import { Renderer, Camera, Transform, Orbit, Program, GLTFLoader, Vec3, TextureLoader, Plane, Mesh } from 'ogl';
...
const smokeVertex = /* glsl */`
  ...
  void main() {
    ...
    // Twist
    float twistPerlin = texture2D(tPerl, vec2(0.5, uv.y * 0.2 - uTime * 0.005)).r;
    float angle = twistPerlin * 10.0;
    newPosition.xz = rotate2D(newPosition.xz, angle);

    // Wind
    vec2 windOffset = vec2(
      texture2D(tPerl, vec2(0.25, uTime * 0.01)).r -0.5,
      texture2D(tPerl, vec2(0.75, uTime * 0.01)).r -0.5
    );
...
const smokeFragment = /* glsl */`
  ...
  void main() {
    ...
    // Smoke
    float smoke = texture2D(tPerl, smokeUv).r;
...

{
    ...
    async function loadInitial() {
        gltf = await GLTFLoader.load(gl, `assets/gltf/bakedModel.glb`);
        addGLTF(gltf);
    ...
    // ———— Smoke ———————————————————————————————————————————————————————————————————————————
    const smokeGeometry = new Plane(gl, {
      width: 1,
      height: 1,
      widthSegments: 16,
      heightSegments: 64
    ...
    const positions = smokeGeometry.attributes.position.data
    for(let i = 0; i < positions.length; i += 3) {
      positions[i + 0] = positions[i + 0] * 1.5
      positions[i + 1] = positions[i + 1] * 6 + 0.5
      positions[i + 2] = positions[i + 2] * 1.5
    }
    smokeGeometry.attributes.position.needsUpdate = true

    // Perlin Tex
    const perlineTexture = TextureLoader.load(gl, {
    ...
    const smokeMesh = new Mesh(gl, { geometry: smokeGeometry, program: smokeProgram })
    smokeMesh.position.y = 4.5
    smokeMesh.setParent(scene)
    ...
    function update(t) {
        ...
        // Smoke Animation
        smokeProgram.uniforms.uTime.value = elapsedTime;
